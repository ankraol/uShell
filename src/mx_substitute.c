...
char *mx_substitute(char *command, t_path_builtin *pwd, t_builtin_command *my_command, t_pid_name **pid_ar) {
    bool ap = false;
    bool iSbr = false;
    bool extr = false;
    ...
    if (j > 0) {
        ...
        if (pid == 0) {
            ...
            for (int i = 0; work[i]; i++) {
                p = work[i];
                for (; p; p = (*p).next) {
                    (*p).command = mx_substitute((*p).command, pwd, my_command, pid_ar);
                    status = mx_redirection((*p).command, pwd, my_command, pid_ar);
                    if (((*p).op == '&' && status == 1)
                        || ((*p).op == '|' && status == 0))
                        {
                            p = (*p).next;
